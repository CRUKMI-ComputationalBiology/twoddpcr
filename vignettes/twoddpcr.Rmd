...
```{r fig.ext='png', fig.width=5, fig.height=4}
kmeansSummary <- plateSummary(plate, cMethod="kmeansMahRain")
head(kmeansSummary)
```
...
A Shiny app is included in the package that allows interactive use of the 
package for ddPCR analysis. This can be run from an interactive R session 
using:
...
This can currently be accessed at [http://shiny.cruk.manchester.ac.uk/twoddpcr/](http://shiny.cruk.manchester.ac.uk/twoddpcr/).
